...
class LCD_ILI9881C final : public esp_panel::drivers::LCD {
public:
    static constexpr esp_panel::drivers::LCD::BasicAttributes BASIC_ATTRIBUTES_DEFAULT = {
        .name = "ILI9881C",
    };

    LCD_ILI9881C(esp_panel::drivers::Bus *bus, int width, int height, int color_bits, int rst_io):
        esp_panel::drivers::LCD(BASIC_ATTRIBUTES_DEFAULT, bus, width, height, color_bits, rst_io)
    {
    }

    LCD_ILI9881C(esp_panel::drivers::Bus *bus, const esp_panel::drivers::LCD::Config &config):
