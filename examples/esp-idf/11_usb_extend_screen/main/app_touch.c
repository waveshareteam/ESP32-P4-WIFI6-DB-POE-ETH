...
static uint16_t app_touch_scale_y(uint16_t y)
{
    return (uint32_t)y * EXAMPLE_LCD_V_RES / BSP_LCD_V_RES;
}
#endif
...
#if CONFIG_IDF_TARGET_ESP32P4
                report.touch_report.data[i].y = app_touch_scale_y(touch_points[i].y);
#else
