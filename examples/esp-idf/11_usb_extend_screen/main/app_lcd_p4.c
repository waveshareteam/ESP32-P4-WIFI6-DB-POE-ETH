...
static esp_lcd_panel_io_handle_t display_io_handle;
...
void app_lcd_draw(uint8_t *buf, uint32_t len, uint16_t width, uint16_t height)
{
    ...
    esp_err_t ret = jpeg_decoder_process(jpgd_handle, &decode_cfg, buf, len, lcd_buffer[buf_index],
...
esp_err_t app_lcd_init(void)
{
    ...
    ESP_RETURN_ON_ERROR(jpeg_new_decoder_engine(&decode_eng_cfg, &jpgd_handle), TAG,
                        "create JPEG decoder failed");

    bsp_display_config_t disp_config = {
        .num_fbs = EXAMPLE_LCD_BUF_NUM,
    };

    ESP_RETURN_ON_ERROR(bsp_display_new(&disp_config, &display_handle, &display_io_handle), TAG,
                        "create display failed");
    ESP_RETURN_ON_ERROR(bsp_display_brightness_init(), TAG, "initialize backlight failed");
    ESP_RETURN_ON_ERROR(bsp_display_backlight_on(), TAG, "turn on backlight failed");
