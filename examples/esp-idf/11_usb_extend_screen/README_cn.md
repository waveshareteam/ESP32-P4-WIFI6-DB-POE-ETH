...
* P4 开发板
   ...
   2. ESP32_P4_WIFI6_DB_TOUCH_LCD_7C 开发板（1024×600）
...
P4 的 USB 宽高自动跟随 menuconfig 中选择的 BSP 屏幕，不能再单独修改。
沿用现有 USB 宽高交换关系：5/7 英寸为 WIDTH=1280、HEIGHT=720；
8/10.1 英寸为 WIDTH=1280、HEIGHT=800。切换屏幕后需重新配置和构建。
重新连接 USB 显示器，让电脑读取更新后的尺寸；编译期检查会拦截 USB 与 LCD 尺寸不一致的配置。
