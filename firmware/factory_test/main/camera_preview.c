...
esp_err_t camera_preview_run(esp_lcd_panel_handle_t panel_handle, const dev_display_lcd_config_t *lcd_cfg)
{
    dev_camera_handle_t *camera_handle = NULL;
    ESP_RETURN_ON_ERROR(esp_board_device_get_handle("camera", (void **)&camera_handle),
    ...
    if (panel_handle == NULL || lcd_cfg == NULL || lcd_cfg->lcd_width == 0 || lcd_cfg->lcd_height == 0) {
        ESP_LOGE(TAG, "Invalid LCD panel/config");
        return ESP_ERR_INVALID_ARG;
    }
