# ESP32-P4-WIFI6-DB-POE-ETH BSP API
...
The board-specific header is `bsp/esp32_p4_wifi6_db_poe_eth.h`.
...
| RTC | `BSP_CAPS_RTC` | 0 |
| Ethernet | `BSP_CAPS_ETHERNET` | 1 |
| Wi-Fi coprocessor | `BSP_CAPS_WIFI` | 1 |

## Ethernet and Wi-Fi

- `bsp_eth_init(&handle)` installs the board RMII MAC and generic PHY driver.
- `bsp_eth_deinit(handle)` uninstalls it and frees MAC/PHY after the application
  has stopped the driver and released netif glue references.
- `bsp/ethernet.h` exposes Ethernet APIs and wiring without display includes.
- The application owns Ethernet netif, event loop, DHCP and bridge policy.
- `BSP_WIFI_*` describes ESP32-C5 SDIO wiring; Hosted configuration is managed
  by the application, referring to `03_wifistation` / `06_eth2ap`.
- Wi-Fi initialization uses the existing ESP Wi-Fi Remote API; no new BSP
  transport or network-policy wrapper is introduced.

See [README](README.md) for wiring, configuration and lifecycle examples.
